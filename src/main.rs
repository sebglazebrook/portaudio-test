...
fn run() -> Result<(), portaudio::Error> {
    ...
    let output_params = portaudio::StreamParameters::new(def_output, CHANNELS, INTERLEAVED, latency);
    ...
    let callback = move |portaudio::DuplexStreamCallbackArgs { in_buffer, out_buffer, frames, time, .. }| {
        ...
        if count_down > 0.0 { portaudio::Continue  } else { portaudio::Complete  }
